...
interface Teil1BAIPhaseProps {
  ...
  startRecording: () => void;
  stopRecording: () => void;
  onAnswersReady: (text: string) => void;
}

export default function Teil1BAIPhase({
  ...
  useEffect(() => {
    if (transcript && !processing && !fallbackMode) {
      onAnswersReady(transcript);
    }
  }, [transcript, processing, fallbackMode, onAnswersReady]);
    ...
    <div style={{ padding: '0 4px' }}>
      <div style={{ textAlign: 'center', marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 13, color: '#94a3b8', margin: 0 }}>
          {step === 'listening' ? 'Hören Sie die Zusammenfassung Ihres Partners' : 'Beantworten Sie die Fragen (1 Minute)'}
        </p>
      </div>
      ...
      {step === 'answering' && !transcript && (
        <>
          <Timer totalSeconds={DURATION.TEIL_1B_ANSWERS} running={!transcript} />
      ...
      {fallbackMode && step === 'answering' && (
        <div style={{
          ...
          <textarea
            placeholder="Ihre Antwort auf die Fragen..."
            style={{
          ...
          <button
            onClick={() => {
              const ta = document.querySelector('.telc-teil1b textarea') as HTMLTextAreaElement | null;
              const val = ta?.value || '';
              if (val.trim()) onAnswersReady(val);
            }}
            style={{
