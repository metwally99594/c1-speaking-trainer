...
export default function Exam() {
  ...
  const finishExam = () => {
    ...
    // Coverage Analysis (Simple keyword matching)
        ...
        <div className="bg-gray-950 border border-gray-900 rounded-3xl p-10 text-center shadow-2xl">
          ...
          <div className="space-y-4 text-gray-400 max-w-lg mx-auto mb-10 text-lg leading-relaxed">
            ...
            <ul className="text-left space-y-2 text-sm">
              <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-blue-500 rounded-full"></span> Teil 1: Freie Präsentation (Ihr Text wird ausgeblendet)</li>
              <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-blue-500 rounded-full"></span> Teil 2: Anschlussfragen vom Prüfer</li>
              <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-blue-500 rounded-full"></span> Auswertung: Abdeckung der Kernpunkte & Wortanzahl</li>
            </ul>
          </div>
          <button 
            onClick={startExam}
        ...
        <div className="space-y-8">
           <div className="bg-gray-950 border border-gray-900 rounded-3xl p-10 shadow-2xl relative overflow-hidden">
             ...
             <div className="flex justify-between items-start mb-8">
               <div className="space-y-1">
                 <span className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">
                   {phase === 'presentation' ? 'Phase 1: Präsentation' : `Phase 2: Fragen (${currentQuestionIdx + 1}/${questions.length})`}
                 </span>
                 <h2 className="text-3xl font-black text-white">
                   {phase === 'presentation' ? 'Bitte präsentieren Sie Ihr Thema.' : questions[currentQuestionIdx]}
                 </h2>
               </div>
             ...
             <div className="bg-black/50 border border-gray-800 rounded-2xl p-6 min-h-[200px] mb-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Live Transkript</span>
                </div>
             ...
             <div className="flex gap-4">
                ...
                >
                  {phase === 'presentation' ? 'Präsentation beenden & Fragen starten' : (currentQuestionIdx < questions.length - 1 ? 'Nächste Frage' : 'Prüfung abschließen')}
                </button>
             </div>
           </div>
           ...
           <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
             <div className="flex items-center gap-2 text-gray-500">
               <AlertCircle size={16} />
               <p className="text-sm">Hinweis: Sprechen Sie deutlich und versuchen Sie, alle vorbereiteten Punkte abzudecken.</p>
             </div>
           </div>
        </div>
      )}
        ...
        <div className="space-y-8 animate-in fade-in zoom-in duration-500">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div className="bg-gray-950 border border-gray-900 p-8 rounded-3xl text-center shadow-xl">
              ...
              <div className="text-xs font-bold text-gray-500 uppercase tracking-widest">Wörter</div>
            </div>
            <div className="bg-gray-950 border border-gray-900 p-8 rounded-3xl text-center shadow-xl">
          ...
          <div className="bg-gray-950 border border-gray-900 rounded-3xl p-10 shadow-xl">
             ...
             <div className="space-y-6">
                <div>
                  <p className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-3">Vollständigkeits-Check</p>
                  <div className="space-y-2">
                    {topic.sentences.map((s, idx) => {
                      const words = s.text.toLowerCase().split(/\s+/).filter(w => w.length > 4);
                      const isMentioned = words.some(w => transcript.toLowerCase().includes(w));
                      return (
                        <div key={idx} className={cn(
                          ...
                          {isMentioned ? <CheckCircle2 size={16} /> : <AlertCircle size={16} />}
          ...
          <div className="flex gap-4">
             ...
             <button 
               onClick={() => { setPhase('intro'); setTranscript(''); setElapsedTime(0); }}
